defmodule BuDuoTravel.Travel do
  ...
  def hotel_list(loc, inn, out) do
    resp = HTTPoison.get!("https://api.sandbox.amadeus.com/v1.2/hotels/search-airport?apikey=prBuA68WLoPMGqJ91KtRArlYap50quAB&location=#{loc}&check_in=#{inn}&check_out=#{out}", [connect_timeout: 50_000, timeout: 50_000, recv_timeout: 50_000])
    data = Poison.decode!(resp.body)
    data["results"]
end
